...
class BillingStatus(Resource):
    ...
    def get(self):
        ...
        try:
            logger.info("[BillingStatus] check user_id=%s", user_id)

            active, sub = is_active(user_id)
            sub_plan_type = getattr(sub, "plan_type", None) if sub else None
            feature_plan = "premium" if active else "free"
            if active:
                plan_label = sub_plan_type if sub_plan_type in {"daily", "monthly"} else "premium"
            else:
                plan_label = "free"

            usage_snapshot = snapshot(user)

            try:
                quiz_remaining, quiz_row = get_remaining_weekly_quizzes(user_id)
            except Exception:
                logger.exception("[BillingStatus] failed to load quiz usage user_id=%s", user_id)
                quiz_remaining, quiz_row = (0, None)

            quiz_used = getattr(quiz_row, "quiz_count", 0) if quiz_row else 0
            quiz_week_key = getattr(quiz_row, "week_key", None) if quiz_row else None
            quiz_limit = "unlimited" if feature_plan == "premium" else FREE_TIER_WEEKLY_QUIZZES
            quiz_remaining_value = "unlimited" if feature_plan == "premium" else max(0, int(quiz_remaining or 0))

            ai_limit = "unlimited" if feature_plan == "premium" else usage_snapshot.get("free_quota")
            ai_remaining = "unlimited" if feature_plan == "premium" else usage_snapshot.get("remaining")

            body = {
                "subscription_status": "active" if active else "inactive",
                "plan": plan_label,
                "plan_type": sub_plan_type,
                "subscription_plan": sub_plan_type,
                "current_period_start": (
                    sub.start_date.isoformat() + "Z" if getattr(sub, "start_date", None) else None
                ),
                "current_period_end": (
                    sub.end_date.isoformat() + "Z" if getattr(sub, "end_date", None) else None
                ),
                "month_key": usage_snapshot.get("month_key"),
                "month_free_limit": usage_snapshot.get("free_quota"),
                "month_used": usage_snapshot.get("used"),
                "month_remaining": usage_snapshot.get("remaining"),
                "usage": {
                    "plan": feature_plan,
                    "plan_type": sub_plan_type if active else None,
                    "ai_generation": {
                        "used": usage_snapshot.get("used"),
                        "limit": ai_limit,
                    ...
                    "quizzes": {
                        ...
                        "week_key": quiz_week_key,
                    },
                },
            }

            logger.info(
                "[BillingStatus] user_id=%s sub_active=%s sub_plan_type=%s -> plan=%s",
                user_id,
                active,
                sub_plan_type,
                plan_label,
...
class VerifyPayment(Resource):
    ...
    def post(self):
        ...
        normalized_status = tx.status
        status_info = None

        if tx.status in {"initiated", "pending"} and tx.provider_ref:
            status_info = check_payment_status_safe(tx, max_retries=1)
            backfill_invoice_id(tx, status_info.get("raw") or {})
            normalized = status_info.get("normalized_state", "pending")

            if normalized == "succeeded":
                activation_payload = status_info.get("raw") or status_info
                activate_subscription_for_transaction(tx, activation_payload or {})
